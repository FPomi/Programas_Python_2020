...
def buscar_pseudonimo(usuarios, datos, pseudonimo_usuario):
    '''devuelve el pseudonimo de la persona con quien el usuario quiere hablar o None si quiere volver al menu'''
    _,_,_,_,_,_,_,likes,chats = datos

    chat_entrar = input ("Elige el nº del chat a entrar, o 0 para volver atrás: ")
    invalido = True
    ...
    if int(chat_entrar) != 0:
        n = 0
        while n < int(chat_entrar):
            if pseudonimo_usuario in usuarios[likes[n]][7]:
                resultado = likes[n]
                n += 1
...
def mostrar_chat(usuarios, pseudonimo_usuario, pseudonimo_match):
    ...
    if usuarios[pseudonimo_usuario][-1][pseudonimo_match] == " ":
        usuarios[pseudonimo_usuario][-1][pseudonimo_match] = pseudonimo_usuario + "_" + pseudonimo_match + ".txt"
        usuarios[pseudonimo_match][-1][pseudonimo_usuario] = pseudonimo_usuario + "_" + pseudonimo_match + ".txt"

        ruta_archivo = pseudonimo_usuario + "_" + pseudonimo_match + ".txt"
    else:
        ruta_archivo = usuarios[pseudonimo_usuario][-1][pseudonimo_match]
...
def mandar_mensajes(usuarios, ruta_archivo, pseudonimo_usuario, pseudonimo_match):
    '''manda mesnajes :)'''
    mensaje = input("Escribir mensaje, o enter para volver al menu de chats: ")
    while mensaje:
...
def menu_chats(pseudonimo_usuario):
    ...
    while seguir_chat:
        ...
        #imprimo el menu
        imprimir_menu(datos, usuarios, pseudonimo_usuario)

        #me fijo a que chat quiere entrar el usuario, o si quiere vovler al menu principal
        resultado = buscar_pseudonimo(usuarios, datos, pseudonimo_usuario)
