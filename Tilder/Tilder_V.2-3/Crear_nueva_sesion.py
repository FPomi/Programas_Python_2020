...
def Pseudonimo(dicc_usuarios):
    ...
    caracter = 0
    while caracter != len(pseudo) or pseudo == "" or pseudo in dicc_usuarios:
        
        if pseudo in dicc_usuarios:
...
def etiqueta_valida(interes, lista_intereses):
    ...
    if len(interes) == 0:
        ...
        while indice < len(interes):
            if interes[indice] == " " or interes[indice] in tildes or (interes[indice].isalpha() == False and interes[indice] != guion) or interes in lista_intereses:
...
def etiquetas_de_interes():
    ...
    val, interes = etiqueta_valida(interes, lista_intereses)
