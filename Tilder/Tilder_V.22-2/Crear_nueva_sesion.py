...
def Pseudonimo(dicc_usuarios):
    ...
    while (pseudo in dicc_usuarios):
        pseudo = input(" EROR. Parece que el Pseudonimo esta en uso (> ~ <) - Intenta con otro: ")
            
    caracter = 0
    while caracter != len(pseudo):
        if ( not pseudo[caracter].isalnum() ) or ( pseudo[caracter].isupper() ):
            if pseudo[caracter] != "_":
                pseudo=input(" EROR. Pseudonimo invalido - Ingrese uno nuevo: ")
                caracter = 0
            else:
                caracter += 1
        else:
...
def contraseña():
    ...
    while len(contraseña) < LARGO_MIN_CONT  or not verf_contraseña(contraseña):
        contra = input(" ERROR. La contraseña no cumple con los requisitos - Ingrese una nueva: ")
...
def ingre_edad():
    ...
    while ed < EDAD_MIN or ed > EDAD_MAX:
        ed = int(input(" Edad fuera de los limites, intentelo de nuevo: "))
...
def ubicacion():
    ...
    latitud = float(input(" ♥Ingrese su latitud en grados decimales [Consulte en Go*gle M*ps de no conocerla]: "))
    coord.append(latitud)
    
    longitud = float(input(" ♥Ingrese su longitud en grados decimales [Consulte en Go*gle M*ps de no conocerla]: "))
